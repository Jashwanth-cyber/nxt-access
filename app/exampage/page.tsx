...
export default function ExamPage() {
 ...
 useEffect(() => {
  if (!initialStateLoaded || !session?.user?.email) return;
  ...
  localStorage.setItem(`exam_state_${session.user.email}`, JSON.stringify(examState));
}, [answers, timeLeft, current, session, initialStateLoaded]);
            ...
            <div className="bg-white rounded-xl shadow p-6 flex flex-col">
                ...
                <div className="grid grid-cols-5 gap-2">
                  {QUESTIONS.map((_, idx) => (
                    <button
                      ...
                      className={`w-8 h-8 rounded border text-sm ${current === idx
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="w-full border border-blue-900 text-blue-900 py-2 rounded hover:bg-blue-50"
                >
                  Submit Assessment
                </button>
                <button
                  onClick={() => router.push("/dashboard")}
                  className="text-sm text-blue-800 border border-blue-800 px-3 py-2 rounded hover:bg-blue-50"
                >
                  ← Back to Dashboard
                </button>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                  {QUESTIONS[current].options.map((opt, idx) => (
                    <button
                      key={opt}
                      className={`w-full py-4 rounded-md border text-left px-4 transition ${answers[current] === opt
